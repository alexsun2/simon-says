...
class SimonWorld extends World {
  ...
  /* TEMPLATE
  FIELDS:              
  ... this.clickedSequence ...                              -- ILoButton
  ... this.flashSequence ...                                -- ILoButton
  ... this.futureSequence ...                               -- ILoButton
  ... this.click ...                                        -- String
  ... this.rand ...                                         -- Random
  ...
  ... this.randomButton() ...                               -- Button

  METHODS FOR FIELDS: 
  ... this.futureSequence.flashSequence(WorldScene) ...     -- WorldScene
  ... this.flashSequence.startsWith(ILoButton) ...          -- boolean
  ... this.clickedSequence.length() ...                     -- int
  ... this.flashSequence.length() ...                       -- int
  ... this.flashSequence.insert(Button) ...                 -- ILoButton
  ... this.futureSequence.remove() ...                      -- ILoButton
  ... this.clickedSequence.insert(Button) ...               -- ILoButton
   */
...
class MtLoButton implements ILoButton {
  ...
  METHODS:
  ... this.insert(Button) ...                               -- ILoButton
  ... this.length() ...                                     -- int
  ... this.remove() ...                                     -- ILoButton
  ... this.buttonIndex(int, int) ...                        -- Button
  ... this.startsWith(ILoButton) ...                        -- boolean
  ... this.flashSequence(WorldScene) ...                    -- WorldScene
  ... this.unflash() ...                                    -- ILoButton
...
class ConsLoButton implements ILoButton {
  ...
  METHODS FOR FIELDS: 
  ... this.rest.insert(Button) ...                          -- ILoButton
  ... this.rest.length() ...                                -- int
  ... this.rest.buttonIndex(int, int) ...                   -- Button
  ... this.first.sameButton(Button) ...                     -- boolean
  ... this.rest.startsWith(ILoButton) ...                   -- boolean
  ... this.first.sameColor(Color) ...                       -- boolean
  ... this.first.drawLit() ...                              -- WorldImage
  ... this.rest.unflash() ...                               -- ILoButton
   */
  ...
  public boolean startsWith(ILoButton buttons) {
    ...
    METHODS ON PARAMETERS:
    ... buttons.length() ...                                  -- int
    ... buttons.buttonIndex(int, int) ...                     -- Button
    ... buttons.remove() ...                                  -- ILoButton
     */
...
class ExamplesSimon {
  ...
  boolean testSimonSays(Tester t) {
    ...
    int sceneSize = 500;
    return starterWorld.bigBang(sceneSize, sceneSize, 0.7);
  }
}